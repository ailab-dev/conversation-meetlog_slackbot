...
app/
  api/
    slack/
      events/route.ts    # Slack Events API 受信（自動収集・RAG・参加検知）
      actions/route.ts   # Block Kit ボタン応答（過去メッセージ収集確認）
      collect/route.ts   # QStash 経由の一括収集処理
lib/
  slack.ts               # 署名検証・メッセージ送信・Block Kit 送信
  notion.ts              # Notion ページ作成・スレッド返信追記
  redis.ts               # 重複防止・自動収集フラグ・Notion ページ ID キャッシュ
...
| 期日管理 | 自然言語の日付（「来週月曜」「3/31」など）を解釈して Notion に保存 |
